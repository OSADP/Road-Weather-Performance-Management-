...
namespace InfloWebRole.Controllers
{
    public class MapController : ApiController
    {
        ...
        public IEnumerable<string> Get()
        {
            ...
            {
                var roadways = srInfloDbContext.Configuration_Roadways.GetAll().Select(r => r.Name + "='" + r.RoadwayId + "'").ToList();
        ...
        private byte[] generateMapMessage(string roadwayid, int startIndex, int linkCount)
        {
            ...
            Configuration_Roadway roadway = srInfloDbContext.Configuration_Roadways.GetAll().Where(r => r.RoadwayId.Equals(roadwayid)).FirstOrDefault();
            if (roadway == null)
                return new byte[0];

            IEnumerable<Configuration_RoadwayMileMarkers> dbMileMarkers = srInfloDbContext.Configuration_RoadwayMileMarkers
                .GetAll().Where(r => r.RoadwayId.Equals(roadway.RoadwayId));

            var dbLinks = srInfloDbContext.Configuration_RoadwayLinks
                .GetAll().Where(l => l.RoadwayId.Equals(roadway.RoadwayId)).ToList()
